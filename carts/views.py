...
#     try:
#         cart_item = CartItem.objects.filter(product=product, cart=cart).first()  # Get the first cart item
...
def remove_cart(request, product_id):
    ...
    if request.user.is_authenticated:
        # If user is authenticated, remove cart items associated with the product and user
        cart_items = CartItem.objects.filter(product=product, user=request.user)
    else:
        # If user is not authenticated, remove cart items associated with the product and cart
        cart = Cart.objects.get(cart_id=_cart_id(request))
        cart_items = CartItem.objects.filter(product=product, cart=cart)

    # Handle each cart item individually
...
def remove_cart_item(request, product_id, cart_item_id):
    try:
        ...
        if request.user.is_authenticated:
            # If user is authenticated, check cart item associated with the user
            cart_item = get_object_or_404(CartItem, id=cart_item_id, user=request.user)
        else:
            # If user is not authenticated, check cart item associated with the session
            cart = Cart.objects.get(cart_id=_cart_id(request))
